...
const WithdrawTable = ({ transactions }: { transactions: any }) => {
...
const {getMerchantName}=useFetchFirestoreData()
            ...
            {sortedData.map((item: any) => (
              ...
              >
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <p className="text-black dark:text-white">{convertTimestampToDateTime(item.timestamp)}</p>
                </div>
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <p className="text-meta-5">
                    {!item.approved ? <button onClick={async()=>{
                        const response= await axios.put('https://api.techarman.me/transactions/withdraw/'+item.id)
...
console.log(response)
                    ...
                    }}  className='bg-blue-500 px-2 py-1 rounded-xl text-white'>Approve</button> : <button className='bg-blue-300 px-2 py-1 rounded-xl text-white' disabled >Approved</button>}
                  </p>
                </div>
              </div>
            ))}
